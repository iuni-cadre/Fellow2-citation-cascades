...
class _AsynchronousTaskQueue:
    ...
    def call(self, fun, **kwargs):
        future = self.thread_pool.submit(fun, **kwargs)
...
class _MLflowInstrumentation:
    ...
    def log_crossvalidator(cv, metrics):
        ...
        if _mlflow is None:
            _MLflowInstrumentation._no_mlflow_warning()
            return
        if _MLflowInstrumentation.usage_logger is None:
            _MLflowInstrumentation.usage_logger = _MLflowUsageLogger()
        run_uuid = _MLflowInstrumentation._get_or_start_mlflow_run().info.run_uuid
        _MLflowInstrumentation.async_task_queue.call(
            fun=_MLflowInstrumentation._fun_call_catch_exception,
            func=_MLflowInstrumentation._log_cv,
            run_uuid=run_uuid,
            tuning=cv,
            metrics=metrics)

    @staticmethod
    def _log_cv(run_uuid, cv, metrics):
        _MLflowInstrumentation.mlflow_client = _MlflowClient()

        est = cv.getOrDefault(cv.estimator)
        epm = cv.getOrDefault(cv.estimatorParamMaps)
        nFolds = cv.getOrDefault(cv.numFolds)
        eva = cv.getOrDefault(cv.evaluator)

        params = _MLflowInstrumentation._get_common_info(cv, epm, eva)
        params['numFolds'] = nFolds
        _MLflowInstrumentation._log_tuning_info(run_uuid, params)

        metric_names = ['avg_'+eva.getMetricName(), 'std_'+eva.getMetricName()]
        metrics = np.stack((np.mean(metrics, axis=0), np.std(metrics, axis=0)))
        _MLflowInstrumentation._log_tuning_runs(run_uuid, est, epm, metric_names, metrics)
    ...
    def log_trainvalidationsplit(tvs, metrics):
        ...
        _MLflowInstrumentation.async_task_queue.call(
            fun=_MLflowInstrumentation._fun_call_catch_exception,
            func=_MLflowInstrumentation._log_tvs,
            run_uuid=run_uuid,
            tuning=tvs,
            metrics=metrics)

    @staticmethod
    def _log_tvs(run_uuid, tvs, metrics):
        _MLflowInstrumentation.mlflow_client = _MlflowClient()

        est = tvs.getOrDefault(tvs.estimator)
        epm = tvs.getOrDefault(tvs.estimatorParamMaps)
        eva = tvs.getOrDefault(tvs.evaluator)

        params = _MLflowInstrumentation._get_common_info(tvs, epm, eva)
        _MLflowInstrumentation._log_tuning_info(run_uuid, params)

        metric_names = [eva.getMetricName()]
        _MLflowInstrumentation._log_tuning_runs(run_uuid, est, epm,
    ...
    @staticmethod
    def _log_tuning_info(run_uuid, params):
        """
    ...
    @staticmethod
    def _log_tuning_runs(parent_run_uuid, est, epm, metric_names, metrics):
        """
        ...
        for index in range(num_models):
            ...
            active_run = _MLflowInstrumentation.mlflow_client.create_run(
                experiment_id=exp_id_for_run, parent_run_id=parent_run_uuid)
